...
export default class ARR1 extends Phaser.Scene {
    ...
    loadScene(x,y,direction,level,dungeon){
        ...
        this.scene.start(level+dungeon[y][x].name,{X: x, Y: y, dg: dungeon});
    }
    ...
    create() {
        ...
        for (const objeto of this.map.getObjectLayer('Cosas').objects) {
            ...
            if (objeto.type === 'Trigger') {
                new Trigger(this,objeto.x, objeto.y,objeto.width, objeto.height,this.player,'ar',this.x,this.y,this.loadScene,'n',this.dungeon);
            }
        }
        
        this.map.removeAllLayers()
        this.map.destroy()
       this.scene.start('arR2')
